...
Our group project is an online calorie and macronutrient tracker that gives people a convenient way to keep track of what they eat every day. The platform will be a web based application where users can make an account manually or through Google Oauth, set their daily calorie and macro targets, and log foods or meals either from a searchable database or by entering custom values.

As users add entries, the app will show how each meal affects their daily totals in real time. To make this clear, we want to include visual summaries like progress bars, pie charts, and other simple displays so people can quickly understand their progress, the tracker will also let users save meals they use often, edit or delete past entries, and export their history as a CSV file if they want to look at their data in more detail.
...
## World Assumptions:
- Users have access to the internet and a suitable computer for running the application.
...
- Food and meal entries are sourced from either user entries or predefined database documents, and are assumed to be sufficiently accurate for tracking purposes.
...
- Log meals into a daily diary (shown in dashboard and history pages)
- Set and edit daily calorie and macronutrient targets.
- Provide real-time daily totals and progress visualizations.
- View nutrition history by day with the option to export data as CSV.
...
- Dashboard / Home – Displays today’s calorie total, macronutrient breakdown, and today’s meals/entries
- Food Search / Library – Page containing list of foods in different categories separated by selectable tabs (custom foods, recommendations, and favorites); search capabilities with a dropdown menu to filter by meal (breakfast, lunch, dinner, snack).
...
## Team meetings:
The team will meet every Tuesday and Thursday at 7:00 PM, and will be held virtually via zoom.
